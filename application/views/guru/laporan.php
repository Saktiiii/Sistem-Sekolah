    ...
    <main class="flex-1 p-6 main-scroll-area">
      <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mt-3" id="akademikTabs" role="tablist">
          ...
          <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#absensi">Laporan Absensi</a></li>
            ...
            <form method="get" action="">
              <select name="kelas_id" onchange="this.form.submit()">
                ...
                <?php foreach ($kelas as $k): ?>
                  <option value="<?= $k->id ?>" <?= ($selected_kelas == $k->id) ? 'selected' : '' ?>><?= $k->nama ?></option>
                <?php endforeach; ?>
              </select>
            </form>
    ...
    <aside class="w-80 bg-white h-screen shadow-md p-6 flex-shrink-0 overflow-y-auto">
      <div class="flex flex-col items-center border-b pb-4 mb-4">
        <img src="<?= base_url('assets/profile1.jpg'); ?>"
          class="w-20 h-20 rounded-full object-cover border-4 border-indigo-100 shadow-md" alt="Profile Picture">
